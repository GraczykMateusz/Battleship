...
void Ship::setShipsPosition(std::vector<int>* ship, unsigned int& widthMax, unsigned int& heightMax, std::vector<std::vector<int>>* vecMap2D) {
	...
	do { // protection against set ship on the ship
		do { // protection against rotate when cannot
			// horizontal move (X)
			if(!shipIsRotated) {
				do { // protection against wrong input (go outside the map) [shipIsntRotated]
					cout << "Horizontal:";
					cin >> moveX;
				} while(shipPositionX + moveX < 0 || (shipPositionX + moveX >= widthMax && moveX != 2) || moveX > 2 || moveX < -1);
				...
				do { // protection against wrong input (go outside the map) [shipIsRotated]
	                                cout << "Horizontal:";
	                                cin >> moveX;
	                        } while(shipPositionX + moveX < 0 || (shipPositionX + (*ship).size() + moveX > widthMax && moveX != 2) || moveX > 2 || moveX < -1);
		...
		} while((!checkIfShipCanRotateX(ship, widthMax) || !checkIfShipCanRotateY(ship, heightMax)) && moveX == 2);	
